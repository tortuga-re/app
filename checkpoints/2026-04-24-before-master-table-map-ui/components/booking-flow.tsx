...
import { TableMapSelector } from "@/components/table-map-selector";
...
export function BookingFlow() {
  ...
  const visibleDays = getVisibleBands(availability);
  const enabledSlots = getEnabledSlots(visibleDays);
  const selectedSlot = enabledSlots.find((slot) => slot.time === selectedTime) ?? null;
  ...
  const activeRoomCode = draft.roomCode || bootstrap?.defaultRoomCode || "";
  const compatibleTableOptions = isTableMapSelectionEnabled
    ? getCompatibleTableOptions(activeRoomCode, draft.pax)
    : [];
  const compatibleTableIds = compatibleTableOptions.map((table) => table.id);
  const hasCompatibleTablePreference = compatibleTableIds.includes(
    draft.tablePreferenceId,
  );
  const selectedTablePreference = isTableMapSelectionEnabled
    ? getTableOptionById(activeRoomCode, draft.tablePreferenceId)
    : null;
  const selectedTablePreferenceSummary = buildTablePreferenceSummary(
    selectedTablePreference,
  );
  const composedCustomerNote = buildTablePreferenceNote(
    draft.note,
    selectedTablePreference,
  );
  ...
  const hasNoAvailableSlots = Boolean(
    availability && !loadingAvailability && enabledSlots.length === 0,
  );
  ...
  useEffect(() => {
    if (!draft.tablePreferenceId) {
      return;
    }

    if (
      !isTableMapSelectionEnabled ||
      !activeRoomCode ||
      !hasCompatibleTablePreference
    ) {
      setDraft((current) =>
        ...
        <>
          ...
          {isTableMapSelectionEnabled && activeRoomCode ? (
            <TableMapSelector
              ...
              pax={draft.pax}
              selectedTableId={draft.tablePreferenceId}
              onSelect={(tablePreferenceId) =>
                setDraft((current) => ({ ...current, tablePreferenceId }))
              }
              onClear={() =>
                setDraft((current) => ({ ...current, tablePreferenceId: "" }))
              }
                      ...
                      {showVisibleWaitlistForm ? (
                        <div className="rounded-[1.5rem] border border-[var(--border)] bg-white/4 p-4">
                          <div className="space-y-2">
                            ...
                            <p className="text-sm leading-6 text-[var(--text-muted)]">
                              Lascia i tuoi dati e la ciurma ti ricontattera&apos;
                              se si libera un tavolo per questa richiesta.
                            </p>
                          </div>
                          ...
                          <div className="mt-4 space-y-3">
                            <label className="flex items-start gap-3 text-sm text-[var(--text-muted)]">
                              ...
                              />
                              <span>
                                Accetto il trattamento privacy per inviare la
                                richiesta in lista d&apos;attesa.
                              </span>
                            </label>
                      ...
                      {visibleWaitlistSuccess ? (
                        <div className="rounded-[1.5rem] border border-[var(--border)] bg-white/4 p-4">
                          ...
                          <div className="mt-3 space-y-2 text-sm leading-6 text-[var(--text-muted)]">
                            ...
                            <p>
                              Qualora si dovessero liberare dei posti sarai
                              contattato/a per confermare la prenotazione.
                            </p>
                          </div>
                        </div>
                      ) : null}
          ...
          {selectedSlot ? (
            <div ref={customerDetailsStepRef} className="panel rounded-[2rem] p-5">
              <div className="space-y-2">
                ...
                <p className="text-sm leading-6 text-[var(--text-muted)]">
                  Ultimo passo: inserisci i tuoi dati e conferma il tavolo.
                </p>
              </div>
              ...
              <div className="mt-4 rounded-[1.4rem] border border-[var(--border)] bg-white/4 px-4 py-3">
                ...
                {selectedTablePreferenceSummary ? (
                  <p className="mt-1 text-sm text-[var(--text-muted)]">
                    Preferenza tavolo:{" "}
                    <span className="text-white">{selectedTablePreferenceSummary}</span>
                  </p>
                ) : null}
              </div>
