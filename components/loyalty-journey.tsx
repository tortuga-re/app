...
import { getHighlightImageUrl, type HighlightContent } from "@/lib/highlight-content";
...
export function LoyaltyJourney({ compact = false, beforeHighlights }: { compact?: boolean; beforeHighlights?: React.ReactNode }) {
  ...
  const demoEditorial: HighlightContent = { id: "demo", eyebrow: "Novità a bordo", title: "Una rotta da non perdere", description: "Scopri il prossimo appuntamento della Ciurma.", cta_label: "Prenota", cta_url: "/prenota", detail_title: "Tutti i dettagli", detail_text: "Scopri tutti i dettagli della serata e prenota il tuo tavolo quando vuoi.", starts_at: "", ends_at: null, background_image_url: null, overlay_color: null, priority: 999 };
  const visibleEditorial = scenario.enabled && scenario.demoEditorial ? demoEditorial : editorial;
  const editorialImageUrl = getHighlightImageUrl(visibleEditorial?.background_image_url);
  const editorialHasCta = Boolean(visibleEditorial?.cta_label.trim() && visibleEditorial.cta_url.trim());
  const editorialHasBookingCta = Boolean(editorialHasCta && visibleEditorial && /prenot|booking|cooperto\.it\/in/i.test(`${visibleEditorial.cta_label} ${visibleEditorial.cta_url}`));
  const [surveyOpen, setSurveyOpen] = useState(false);
  const [editorialLinkOpen, setEditorialLinkOpen] = useState(false);
  const [editorialDetailsOpen, setEditorialDetailsOpen] = useState(false);
  ...
  return <section className="loyalty-journey">
    ...
    {!compact ? <>
      ...
      <DragCarousel className="snap-slides" label="Contenuti in evidenza" resetKey={`${pathname}:${visibleEditorial?.id ?? ""}`}>
        {visibleEditorial ? <article className="feature-slide editorial-slide" style={editorialImageUrl ? { backgroundImage: `linear-gradient(${visibleEditorial.overlay_color ?? "rgba(15,18,16,.62)"}, ${visibleEditorial.overlay_color ?? "rgba(15,18,16,.62)"}), url(${editorialImageUrl})` } : undefined}><div className="slide-icon"><Anchor size={20} /></div><p>{visibleEditorial.eyebrow}</p><h3>{visibleEditorial.title}</h3><span>{visibleEditorial.description}</span><div className="slide-actions">{editorialHasCta && (!hasUpcomingReservationSoon || !editorialHasBookingCta) ? editorialCtaExternal ? <button type="button" onClick={() => setEditorialLinkOpen(true)}>{visibleEditorial.cta_label} <ChevronRight size={16} /></button> : <a href={visibleEditorial.cta_url}>{visibleEditorial.cta_label} <ChevronRight size={16} /></a> : null}{visibleEditorial.detail_text?.trim() ? <button type="button" onClick={() => setEditorialDetailsOpen(true)}>Scopri di più <ChevronRight size={16} /></button> : null}</div></article> : null}
    ...
    {editorialDetailsOpen && visibleEditorial?.detail_text?.trim() ? <div className="editorial-details-modal" role="dialog" aria-modal="true" aria-labelledby="editorial-details-title" onClick={() => setEditorialDetailsOpen(false)}><section style={editorialImageUrl ? { backgroundImage: `linear-gradient(${visibleEditorial.overlay_color ?? "rgba(15,18,16,.62)"}, ${visibleEditorial.overlay_color ?? "rgba(15,18,16,.62)"}), url(${editorialImageUrl})` } : undefined} onClick={(event) => event.stopPropagation()}><button type="button" className="editorial-details-close" onClick={() => setEditorialDetailsOpen(false)} aria-label="Chiudi approfondimento"><X /></button><h2 id="editorial-details-title">{visibleEditorial.detail_title?.trim() || visibleEditorial.title}</h2><p className="whitespace-pre-line">{visibleEditorial.detail_text}</p>{editorialHasCta ? editorialCtaExternal ? <button type="button" className="editorial-details-action" onClick={() => { setEditorialDetailsOpen(false); setEditorialLinkOpen(true); }}>{visibleEditorial.cta_label} <ChevronRight size={16} /></button> : <a href={visibleEditorial.cta_url} className="editorial-details-action">{visibleEditorial.cta_label} <ChevronRight size={16} /></a> : null}</section></div> : null}
