...
export function MinimalHomeScreen() {
  ...
  const beforeHighlights = <>
    ...
    {liveGame?.active_game ? <section className="game-context-card"><div className="game-context-heading"><div className="context-icon"><Wifi aria-hidden="true" /></div><div><p className="minimal-eyebrow">Come giocare</p><span>I passaggi vanno eseguiti in ordine</span></div></div><p className="game-wifi-label">COLLEGATI AL WI-FI</p><div className="game-wifi-row"><span><Wifi aria-hidden="true" />TORTUGA</span><span><KeyRound aria-hidden="true" />PERLANERA</span></div><a href={liveGames[liveGame.active_game].url} target="_blank" rel="noreferrer">POI CLICCA QUI <ChevronRight /></a></section> : null}
  </>;
