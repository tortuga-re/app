...
export function CiurmaTabs({ initialTab = "rewards" }: { initialTab?: Tab }) {
  ...
  return <div className="space-y-5">
    ...
    {tab === "ranks" ? <div id="ranghi" className="hash-scroll-target space-y-5">
      <DragCarousel className="rank-slides" label="Ranghi Tortuga">{tortugaRanks.map((rank) => { const reached = isUserRegistered && (getRankIndex(rank.id) <= getRankIndex(active.id)); return <article key={rank.id} className={reached ? "reached" : ""}><RankBadge rank={rank.id} label={rank.label} size={66} /><p>{reached ? "Rango conquistato" : "Prossimo traguardo"}</p><h2>{rank.label}</h2><span>{rank.description}</span><ul className="my-3.5 space-y-1.5 text-left border-t border-[rgba(216,176,106,0.15)] pt-3">{rank.privileges.map((p, idx) => <li key={idx} className="flex items-start gap-1.5 text-[0.74rem] text-[var(--text-muted)] leading-relaxed"><span className="text-[var(--accent-strong)] text-[10px] shrink-0 mt-0.5">⚔</span><span>{p}</span></li>)}</ul><dl><div><dt>Visite</dt><dd>{rank.visits}</dd></div><div><dt>Dobloni raggiunti</dt><dd>{rank.points}</dd></div></dl></article>; })}</DragCarousel>
