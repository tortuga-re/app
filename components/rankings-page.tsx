...
export function RankingsPage() {
  ...
  return <main className="minimal-page rankings-page pb-28"><div className="minimal-overlap-sheet rankings-sheet"><header className="overlap-sheet-intro"><p className="minimal-eyebrow">La plancia della ciurma</p><h1>Classifiche</h1><p>Le squadre, le Leggende e i piatti che hanno conquistato il Tortuga.</p></header><div className="space-y-6">
    <section><div className="mt-1 grid grid-cols-3 gap-2">{(Object.keys(eveningLabels) as Evening[]).map((key) => <button key={key} type="button" onClick={() => setEvening(key)} className={evening === key ? "minimal-primary py-2 text-xs" : "profile-edit-trigger py-2 text-xs"}>{eveningLabels[key]}</button>)}</div><LeaderboardPanel icon={<Trophy size={15} />} title="Vincitori del Tortuga" subtitle="Le squadre con più serate conquistate"><LeaderboardRows loading={loading} items={selectedWinners.slice(0, 30).map((winner, index) => ({ id: winner.id, position: index + 1, name: winner.team_name, value: `${winner.wins} ${winner.wins === 1 ? "vittoria" : "vittorie"}` }))} empty="La prossima squadra vincitrice sara qui." /></LeaderboardPanel></section>
    <LeaderboardPanel icon={<Crown size={15} />} title="Hall of Legends" subtitle="Il registro storico dei pirati leggendari del Tortuga che hanno superato 100 dobloni"><LeaderboardRows loading={loading} items={legends.map((legend, index) => ({ id: `${legend.nickname}-${legend.legend_number}`, position: legend.legend_number || index + 1, name: legend.nickname, value: legend.real_name ?? "" }))} empty="Nessun pirata ha ancora inciso il proprio nome nella storia..." /></LeaderboardPanel>
    <section><div className="mt-1 grid grid-cols-2 gap-2 sm:grid-cols-4">{foodCategories.map((category) => <button key={category.id} type="button" onClick={() => setFoodTab(category.id)} className={foodTab === category.id ? "minimal-primary py-2 text-xs" : "profile-edit-trigger py-2 text-xs"}>{category.label}</button>)}</div><LeaderboardPanel icon={<UtensilsCrossed size={15} />} title="Classifica piatti" subtitle="Classifica piatti degli ultimi 7 giorni"><LeaderboardRows loading={loading} items={(selectedFood?.items ?? []).map((item) => ({ id: `${item.position}-${item.name}`, position: item.position, name: item.name, value: String(item.quantity) }))} empty="La classifica si aggiornera con le prossime comande." /></LeaderboardPanel></section>
  </div></div></main>;
}
...
function LeaderboardRows({ items, empty, loading = false }: { items: { id: string; position: number; name: string; value: string }[]; empty: string; loading?: boolean }) {
  if (loading) {
    return <div className="min-h-[140px] space-y-2 flex flex-col justify-center animate-pulse"><div className="h-10 rounded-[0.8rem] bg-[#f0e9de]/60 w-full" /><div className="h-10 rounded-[0.8rem] bg-[#f0e9de]/60 w-full" /><div className="h-10 rounded-[0.8rem] bg-[#f0e9de]/60 w-full" /></div>;
  }
