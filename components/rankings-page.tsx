...
export function RankingsPage() {
  ...
  return <main className="minimal-page rankings-page pb-28"><div className="minimal-overlap-sheet rankings-sheet"><header className="overlap-sheet-intro"><p className="minimal-eyebrow">La plancia della ciurma</p><h1>Classifiche</h1><p>Le squadre, le Leggende e i piatti che hanno conquistato il Tortuga.</p></header><div className="space-y-6">
    ...
    <section><div className="mt-1 grid grid-cols-3 gap-2">{foodCategories.map((category) => <button key={category.id} type="button" onClick={() => setFoodTab(category.id)} className={foodTab === category.id ? "minimal-primary py-2 text-xs" : "profile-edit-trigger py-2 text-xs"}>{category.label}</button>)}</div><LeaderboardPanel icon={<UtensilsCrossed size={15} />} title="Classifica piatti" subtitle="Classifica piatti degli ultimi 7 giorni"><LeaderboardRows items={(selectedFood?.items ?? []).map((item) => ({ id: `${item.position}-${item.name}`, position: item.position, name: item.name, value: String(item.quantity) }))} empty="La classifica si aggiornera con le prossime comande chiuse." /></LeaderboardPanel></section>
  </div></div></main>;
}
