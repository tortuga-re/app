...
import { createContext, useCallback, useContext, useEffect, useState, type RefCallback } from "react";
...
const PirateSlotMenuGate = dynamic(
  () => import("@/features/pirate-slot/components/PirateSlotMenuGate").then((module) => module.PirateSlotMenuGate),
  { ssr: false },
);
...
export function MenuOverlayProvider({ children }: { children: React.ReactNode }) {
  ...
  const menuCtaRef = useCallback<RefCallback<HTMLElement>>(() => undefined, []);
  ...
  useEffect(() => {
    ...
    const showAfterInteraction = () => {
      ...
      timer = window.setTimeout(() => setAdventureOpen(true), 2_000);
    };
  ...
  return <MenuContext.Provider value={{ openMenu, openMenuDirect, menuCtaRef }}>
    ...
    {gateOpen ? <PirateSlotMenuGate open entryMode={gateMode} onClose={() => setGateOpen(false)} onOpenMenu={openMenuDirect} /> : null}
